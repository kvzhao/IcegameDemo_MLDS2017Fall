#include "sqice_ssf.hpp"
#include <math.h>
#include <assert.h>

/*
    What is an episode? (When reset is called?)
*/

SQIceGameSSF::SQIceGameSSF (INFO info) : sim_info(info) {
    model.init(sim_info);
    latt.init(sim_info);
    ...
    // system size is defined by const in header
    N = NUM_OF_SPINS;
    L = LSIZE;
    ...
    std::cout << "[GAME] SQIceGameSSF is created.\n";
}

bool SQIceGameSSF::TimeOut() {
...
void SQIceGameSSF::UpdateConfig() {
    update_state_to_config();
}

void SQIceGameSSF::restore_config_to_state() {
    state_t = ice_config.Ising;
    state_tp1 = ice_config.Ising;
    state_0 = ice_config.Ising;
}

void SQIceGameSSF::InitModel() {
    model.set_J1(J1);
...
void SQIceGameSSF::SetTemperature(double T) {
    if (T >= 0.0) kT = T;
    std::cout << "[GAME] Set temperature kT = " << kT << "\n";
}

void SQIceGameSSF::MCRun(int mcSteps) {
    /*
...
int SQIceGameSSF::Start(int init_site) {
    set_agent_site(init_site);
    return agent_site;
}

void SQIceGameSSF::ClearBuffer() {
    clear_all();
    same_ep_counter++;
}

// Reset == Restart because of it implementation.:
int SQIceGameSSF::Restart(int init_site) {
    ClearBuffer();
    Start(init_site);
    same_ep_counter = 0;
    num_episode++;
...
int SQIceGameSSF::GetStartPoint() {
    if (agent_site_trajectory.size() != 0) {
        // TODO: use assert
        if (init_agent_site != agent_site_trajectory[0]) {
            std::cout << "[SQIceGameSSF] Sanity check fails! init_agent_site != trajectory[0]!\n";
        }
    }
...
// member functions 
vector<double> SQIceGameSSF::Metropolis() {
    ...
    if (dE == 0.0) {
        if (dd != 0.0) {
            std::cout << "[SQIceGameSSF]: State has no energy changes but contains defects! Sanity checking fails!\n";
        }
        is_accept = true;
        rets[0] = ACCEPT_VALUE;
    } else {
...
void SQIceGameSSF::flip_site(int site) {
...
void SQIceGameSSF::flip_along_traj(const vector<int> &traj) {
...
void SQIceGameSSF::set_agent_site(int site) {
    if (site >= 0 && site < N) {
        agent_site = site;
        // If starting point
        if (agent_site_trajectory.size() == 0) {
...
int SQIceGameSSF::get_agent_site() {
    return agent_site;
}

int SQIceGameSSF::get_spin(int site) {
    int spin = 0;
    if(site >= 0 && site < N) {
        spin = state_t[site];
    }
...
int SQIceGameSSF::get_agent_spin() {
    return get_spin(agent_site);
}

int SQIceGameSSF::coord2site(int coord) {
    if (coord > 0 && coord <= N) {
        return coord - 1;
    } else {
        return NULL_SITE;
...
int SQIceGameSSF::site2coord(int site) {
    if (site >= 0 && site < N) {
        return site + 1;
    } else {
        return NULL_COORD;
...
vector<double>SQIceGameSSF::SSF(int coord) {
    vector<double> rets(4);
    int site = coord2site(coord);

    agent_site = goflip(site);

    // Make sure agent_site == site
    assert(agent_site == site);

    // TODO: Thinking, could we flip back?
    if (canvas_traj_map[site] == EMPTY_MAP_VALUE) {
        canvas_traj_map[site] = OCCUPIED_MAP_VALUE;
    } else if (canvas_traj_map[site] == OCCUPIED_MAP_VALUE) {
        // flip back, erase
        canvas_traj_map[site] = EMPTY_MAP_VALUE;
    }
    ...
    // Now, the rets[0] is just a dummy value.
    rets[0] = ACCEPT_VALUE;
    rets[1] = dE; 
    rets[2] = dd;
    rets[3] = dC;
...
vector<double> SQIceGameSSF::Draw(int dir_idx) {
...
int SQIceGameSSF::goflip(int site) {
    // This function is an analogy to go(int dir)
    int old_site = agent_site;
    int new_site = site;
    set_agent_site(new_site);
    // Now, agent_site is new_site
    // and push site into trajectory list.

    // action_index ~ coordinate (except 0)
    int action_index = site2coord(site);

    // counters increment
    ep_action_counters[action_index]++;
    ep_action_list.push_back(action_index);
    action_statistics[action_index]++;

    agent_map[old_site] = EMPTY_MAP_VALUE;
    agent_map[new_site] = OCCUPIED_MAP_VALUE; 

    // repeat flipping same site is allowed
...
int SQIceGameSSF::how_to_go(int site) {
    int dir = get_direction_by_sites(agent_site, site);
    return dir;
}

int SQIceGameSSF::icemove(bool same_spin) {
    int site = NULL_SITE;
...
vector<int> SQIceGameSSF::get_neighbor_sites() {
...
vector<int> SQIceGameSSF::get_neighbor_spins() {
    vector<int> neighbors(6);
...
vector<int> SQIceGameSSF::get_neighbor_candidates(bool same_spin) {
...
int SQIceGameSSF::get_neighbor_site_by_direction(int dir) {
    int site = agent_site;
    if (dir == 0) {
            site = latt.NN[site][0];
    } else if (dir == 1) {
...
int SQIceGameSSF::get_direction_by_sites(int site, int next_site) {
...
object SQIceGameSSF::GetEnergyMap() {
    for (uint i = 0; i < N; i++) {
        double se = 0.0;
...
// ###### private function ######
double SQIceGameSSF::_cal_energy_of_state(const vector<int> & s) {
    double eng = 0.0;
    for (uint i = 0; i < N; i++) {
        double se;
        if (latt.sub[i] == 1) {
...
double SQIceGameSSF::_cal_energy_of_site(const vector<int> &s, int site) {
    double se = 0.0;
    int i = site;
    if (latt.sub[i] == 1) {
        se = s[i] * (s[latt.NN[i][0]]
...
double SQIceGameSSF::_cal_defect_density_of_state(const vector<int> & s) {
    double dd = 0.0;
    for (uint i = 0; i < N; ++i) {
        if (latt.sub[i] == 1) {
...
double SQIceGameSSF::_cal_mean(const vector<int> &s) {
    return 1.0* std::accumulate(s.begin(), s.end(), 0LL) / s.size();
}

double SQIceGameSSF::_cal_stdev(const vector<int> &s) {
...
void SQIceGameSSF::_print_vector(const vector<int> &v) {
    std::cout << "[";
    for (const auto &i: v)
        std::cout << i << ", ";
    std::cout << "]\n";
...
bool SQIceGameSSF::_is_visited(int site) {
    bool visited = false;
...
bool SQIceGameSSF::_is_start_end_meets(int site) {
    return (site == init_agent_site) ? true : false;
}

bool SQIceGameSSF::_is_traj_continuous() {
    bool cont = true;
...
bool SQIceGameSSF::_is_traj_intersect() {
    /*
        Not complete
    */
    bool meet = false;
...
int SQIceGameSSF::_cal_config_t_difference() {
    int diff_counter = 0;
...
int SQIceGameSSF::_count_config_difference(const vector<int> &c1, const vector<int> &c2) {
